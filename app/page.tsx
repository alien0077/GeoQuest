...
type MapColorMode = "country" | "revealed";
...
function WorldCountryLayer({ className = "", highlightedNames = [], revealedNames = [], selectedName, muted = false, preserveAspectRatio = "none", colorMode = "country", focusContinent, onCountrySelect, onCountryPositions }: { className?: string; highlightedNames?: string[]; revealedNames?: string[]; selectedName?: string; muted?: boolean; preserveAspectRatio?: "none" | "xMidYMid meet"; colorMode?: MapColorMode; focusContinent?: string; onCountrySelect?: (country: MapCountrySelection) => void; onCountryPositions?: (positions: Record<string, MapPosition>) => void }) {
  ...
  return <svg className={`country-outline-layer ${className}`} viewBox="0 0 1000 520" preserveAspectRatio={preserveAspectRatio} role="img" aria-label="世界各國國界輪廓">
    ...
    {paths.map((country) => {
      ...
      const hasCountryColor = colorMode === "country" || revealedNames.includes(countryName);
